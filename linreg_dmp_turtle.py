...
class DMP(object):
    ...
    def __init__(self,w,x_asis_of_turtle,y_asis_of_turtle, pastor_mod = False):
####################################################
        #             Data Collection              #
        ############################################
...
#########################################################################################################################################################
    ...
    def spring_damping_for_sin(self):
        ...
        #Think of it as the slope of the function as it goes through
            ...
            return self.k * (self.y_asis_of_turtle) - self.k * (self.x0) * self.s + self.k
    ...
    def duplicate_for_sin(self):
            ...
            original_matrix_1 = np.vstack((np.zeros([1, (self.goal*10)], dtype = int), (self.y_asis_of_turtle)))
    ...
    def reproduction_for_sin(self, o = None, shape = True, avoidance=False, verbose=0):
        ...
        x = copy.copy(self.y_asis_of_turtle)
        ...
        while t < self.step:
            ...
            if ti % self.step > 0:
                temp_matrix_of_x1 = np.append(copy.copy(x),copy.copy(self.y_asis_of_turtle))
                original_matrix_1 = np.append(copy.copy(self.y_asis_of_turtle),copy.copy(temp_matrix_of_x1))
                original_matrix_2 = np.append(copy.copy(self.y_asis_of_turtle),copy.copy(temp_matrix_of_x2))

            #return the matrix as array when returning
            return np.array(self.y_asis_of_turtle), np.array(x), np.array(original_matrix_1)


    def obstacle_for_sin(self, o, original_matrix_1):

        if self.y_asis_of_turtle.ndim == 1:
            self.y_asis_of_turtle = self.y_asis_of_turtle[np.newaxis, np.newaxis, :]
        if original_matrix_1.ndim == 1:
            original_matrix_1 = original_matrix_1[np.newaxis, np.newaxis, :]

        C = np.zeros_like(self.y_asis_of_turtle)
        R = np.array([[np.cos(np.pi / 2.0), -np.sin(np.pi / 2.0)],
                        [np.sin(np.pi / 2.0),  np.cos(np.pi / 2.0)]])

        for i in xrange(self.y_asis_of_turtle.shape[0]):
            for j in xrange(self.y_asis_of_turtle.shape[1]):
                obstacle_diff = o - self.y_asis_of_turtle[i, j]
...
#########################################################################################
    ...
    y_asis_of_turtle = [None]
    dmp = DMP(w,x_asis_of_turtle,y_asis_of_turtle,True)

    dmp.x_asis_of_turtle = x_coords
    dmp.y_asis_of_turtle = y_coords
